...
class CvrTeacherMultiTask(torch.nn.Module):
    def __init__(
        ...
        self.mmoe = EsmmOuterProductEmbFea(
...
class CvrStudentMultiTaskLitModel(pl.LightningModule):
    ...
    def __init__(self, 
        ...
        self.save_hyperparameters(ignore=['model'])
        # self.automatic_optimization = False
        self.cvr_teacher = CvrTeacherMultiTaskLitModel.load_from_checkpoint('./logs/train/new_teacher/ccp_da.ckpt', model=CvrTeacherMultiTask(embedding_layer=AlldataEmbeddingLayer(batch_type=batch_type)))
...
class CvrStudentMultiTaskLoss(nn.Module):
    def __init__(self, 
                 ...
                 cvr_loss_proportion: float = 1, 
                 ctcvr_loss_proportion: float = 0.1,
                 dis_loss_ratio: float = 1,
                 ):
        super().__init__()
        self.dis_loss_ratio = dis_loss_ratio
    ...
    def caculate_loss(self, p_ctr, p_cvr, p_cvr_1, p_ctcvr, y_ctr, y_cvr, y_cvr_s):
        ...
        loss_cvr = torch.mean(y_ctr*loss_cvr_click + self.dis_loss_ratio*(1-y_ctr)*loss_cvr_unclick)
...
class CvrStudentAllSpaceLoss(nn.Module):
    def __init__(self, 
                 ctr_loss_proportion: float = 1, 
                 cvr_loss_proportion: float = 1, 
                 ctcvr_loss_proportion: float = 0.1,
...
class NewOuterProductFea(torch.nn.Module):
    def __init__(self, input_dim, expert_num, task_num, expert_dims, tower_dims, expert_dropout, tower_dropout):
        ...
        self.tower_pred = torch.nn.ModuleList([
                nn.Linear(self.tower_dims[-1]*5, 1) # Add .cuda() to move the model to GPU
                for _ in range(self.task_num)
        ])
        self.tower_pred[0] = nn.Linear(self.tower_dims[-1], 1)
...
class CvrNewMultiTask(torch.nn.Module):
    def __init__(
        self,
        embedding_layer: AlldataEmbeddingLayer,
        task_num: int = 3,
...
class CvrNewMultiTaskLitModel(pl.LightningModule):
    def __init__(self, 
                 model:torch.nn.Module,
                 loss:torch.nn.Module, 
                 lr:float, 
    ...
    def training_step(self, batch, batch_idx):
       ...
       # oversampling and caculate da loss
        source_representations = tower_fea[2][click==1]
        target_representations = tower_fea[2][click==0]
        source_sampling_weight = torch.ones(len(source_representations))
        indices = torch.multinomial(source_sampling_weight, len(click[click==0]), replacement=True)
        real_S = source_representations[indices]
        real_T = target_representations
...
class CvrNewMultiTaskLoss(nn.Module):
    def __init__(self, 
                 ctr_loss_proportion: float = 1, 
                 cvr_loss_proportion: float = 1, 
                 ctcvr_loss_proportion: float = 0.1,
    ...
    def caculate_loss(self, p_ctr, p_cvr, p_ctcvr, y_ctr, y_cvr):
        loss_ctr = torch.nn.functional.binary_cross_entropy(p_ctr, y_ctr, reduction='mean')
        loss_cvr_click = torch.nn.functional.binary_cross_entropy(p_cvr, y_cvr, reduction='none')
        loss_cvr_unclick = torch.nn.functional.binary_cross_entropy(p_cvr, p_ctcvr, reduction='none')
        loss_cvr = torch.mean(y_ctr*loss_cvr_click + 0.5*(1-y_ctr)*loss_cvr_unclick)
