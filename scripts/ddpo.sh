...
-m data=ccp,fr,us,nl,es experiment=ddpo_experiment test=True data.debug=False trainer.max_epochs=1 trainer.min_epochs=1 >logs/out.log 2>&1 & \

nohup python src/train.py \
-m data=in experiment=ddpo_experiment,ukd_experiment test=True data.debug=False trainer.max_epochs=1 trainer.min_epochs=1 >logs/out.log 2>&1 & \
